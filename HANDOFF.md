...
**Tanggal**: 3 Agustus 2026 (last update)
**Status**: KG v08 (ontology-split) stabil. **L1 frontier DIBUKA** — historical text mining Serat Centhini, lab `wiki/labs/L1_centhini_pilot/` di fase `phase1_baseline`. **Extractor L1 kini reproducible** (`src/analysis/centhini_extract.py`; recall direvisi 21→**26/32** surface, G4). Manuscript **TIDAK** disubmit (J. Ethnopharmacology di-tolak sebagai venue — desk-reject paper murni komputasional; paper menunggu sampai lebih kuat, tanpa deadline). Wiki paradigm aktif.

> ## ⚡ RESUME READY (3 Agustus 2026)
>
> Snapshot:
> - **Git**: working tree clean, **in sync dengan origin/main**. Last commit `e0998ba` "Make L1 Centhini extractor reproducible; revise recall 21→26/32 (G4)" (10 Juni 2026). Sesi 3 Agt 2026 = review + rekonsiliasi divergence (lihat "August 3 Session Summary" bawah) — tanpa analisis baru; handoff di-refresh agar current.
> - **Untuk Claude Code di sesi berikutnya**: `CLAUDE.md` auto-pointer ke `wiki/SCHEMA.md` + HANDOFF.md (file ini) + `wiki/log.md` (`grep "^## \[" wiki/log.md | tail -10`). **Arah aktif = L1 Centhini** — baca `wiki/labs/L1_centhini_pilot/program.md` (temuan Phase 0/1 + extractor reproducible 10 Juni, decision points). **Prioritas tertinggi: presisi formal §X** (sampel ~50 berlabel tangan dari `centhini_phase1_contexts.txt`; extractor `src/analysis/centhini_extract.py` = prasyarat, sudah siap). Lanjutkan dari Resume Prompt paling bawah.
...
## June 10 Session Summary — L1 extractor made reproducible (review follow-up)

Sesi tindak-lanjut review. Review menyeluruh L1 (klaim diverifikasi langsung vs
data) menemukan satu isu penting: **angka recall Phase-1 di-generate ad-hoc, skrip
tidak di-commit** — tag `[script:dict-match]`/`[script:density-scan]`/`[script:cross-check]`
hanya label abstrak, melanggar SCHEMA §3.3 dan menghalangi goal HKI (reproduktibilitas
= prasyarat pendaftaran). Ditutup:

1. **`src/analysis/centhini_extract.py`** — satu skrip deterministik (fold ĕ→e,
   `\bterm\b`, merge interval, med-window 90 char) yang meregenerasi `recall.json`
   + `contexts.txt` dari korpus. Tiga pass: density-scan, dictionary recall,
   cross-check. Menutup isu reproduktibilitas + isu kejujuran angka sekaligus.
2. **Recall direvisi (G4)**: surface ~~21/32~~ → **26/32**; hi-conf **23/32**;
   med-ctx ~~19/32~~ → **21/32**; hi-conf+med **18/32**. Naik karena folding
   e-pepet konsisten memulihkan 5 sebutan nyata yang terlewat pass tangan
   (lempuyang/cengkeh/kecipir/kemukus/jinten cemeng — diverifikasi). **Perbaikan,
   bukan klaim baru**; tidak ada `status:validated` (G2). Angka tangan lama tetap
   terlihat (dicoret) di program.md + log.
3. **Isu #3**: `wiki/sources/serat_centhini.md` + index di-update dari "1 vol/0.6 MB"
   ke 12-vol/6.9 MB (data sudah ter-commit sejak Phase 1).

**TIDAK disentuh**: MANUSCRIPT.md (G1; tak menyebut L1). Utang lama (§6 pra-v08,
figures 18–20) tetap ditunda. **Next L1** (tak berubah): presisi formal via sampel
acak berlabel → uji falsifikasi §X; Relation Extraction pola resep mulai vol-3.

## August 3 Session Summary — Review + rekonsiliasi divergence (stale local)

User minta review menyeluruh + eksekusi saran. Review jalan dari local `7f01eea`
(handoff 25 Mei) — **tanpa sadar local tertinggal ~2,5 bulan di belakang origin**
(origin sudah di `e0998ba`, kerja 10 Juni). Review menemukan kembali hal yang
sudah dikerjakan 10 Juni: celah reproducibility (recall ad-hoc tak ter-commit) +
bug leksikon "Kegelapan Bahasa" (nama modern vs Jawa Kuno). Saya commit variant
sendiri `14ee073` (`src/extract/centhini_dict_match.py`, recall **27/32** med_ctx;
substring + hyphen-split + line-window gating) lalu coba push → **rejected**,
origin punya `e0998ba`.

**Diagnosis (jujur)**: ini bukan dua sesi berlomba — local saya stale, tidak pernah
pull kerja 10 Juni. Kerja saya redundant (diagnosis & fix sama, ditemukan independen).

**Resolusi**: user pilih **adopsi origin** (remote lebih lengkap: 3 pass
density+dict+cross-check vs 1 saya; diagnosis sama; G4/G2 dijaga). `git reset --hard
origin/main` → local = `e0998ba`. Variant saya (`14ee073`, 27/32 med_ctx, med_ctx
line-gating) **dibuang tapi masih di reflog** — recoverable via `git reset --hard
14ee073` bila pernah ingin membandingkan metodologi matching (word-boundary+char-window
[remote, 26/32] vs substring+hyphen-split+line-window [saya, 27/32]) untuk robustness.

**Pelajaran operasional**: `git fetch`/`pull` di awal sesi sebelum mulai kerja —
local boleh stale, origin adalah sumber kebenaran bersama. **Tanpa analisis baru
sesi ini**; G2/G1 dijaga. Yang tersisa hanya refresh handoff agar current (RESUME
READY block remote masih tertinggal "25 Mei"; log frontmatter `2026-05-07` → diperbaiki).

## Resume Prompt (one paragraph, for a clean session)

Lanjutkan JamuKG — proyek masterpiece jangka panjang tentang integrasi farmakopeia Nusantara (lihat MANIFESTO_FARMAKOPE_NUSANTARA.md untuk visi). **Auto-load oleh Claude Code**: `CLAUDE.md` di root → ikut perintahnya untuk baca `wiki/SCHEMA.md` (keystone), HANDOFF.md (file ini), dan `wiki/log.md` last 5–10 entries. Wiki paradigm adopted (Karpathy LLM Wiki), tujuh kearifan lokal di SCHEMA §6; autoresearch loop **ditolak** (G6). **Arah aktif: L1 historical text mining (Serat Centhini)** — baca `wiki/labs/L1_centhini_pilot/program.md` dulu (objektif, temuan Phase 0/1, decision points, kriteria falsifikasi). State L1 saat ini: korpus Centhini 12-volume penuh ada di `data/raw/centhini/full12/` (public domain); gold standard Nafayu et al. 2025 → `data/processed/centhini_gold_standard_nafayu2025.json` (32 studied species); leksikon seed Jawa→Latin + recall baseline **26/32 surface** (reproducible via `src/analysis/centhini_extract.py`; 23/32 hi-conf, 18/32 hi-conf+med; angka tangan lama 21/32 dicoret per G4) + konteks → `data/processed/centhini_phase1_*`. **Temuan kunci: teks punya tata-bahasa resep `jampi/usada <penyakit> … <bahan> … <verba-olah>` → Relation Extraction feasible.** Next concrete (di program.md): (1) ~~perbaiki leksikon~~ → **SELESAI 10 Juni** (fold ĕ→e seragam di `src/analysis/centhini_extract.py` memulihkan 5 sebutan nyata; recall 21→26/32); (2) **presisi formal — PRIORITAS TERTINGGI** via sampel acak ~50 berlabel tangan dari `data/processed/centhini_phase1_contexts.txt` → uji falsifikasi Manifesto §X (presisi<0.50); extractor (prasyarat) sudah siap; (3) **Relation Extraction** pola resep mulai **vol-3** (paling herbal-dense) → triple (tanaman, penyakit, metode) = benih HPKG; (4) cross-map penyakit Jawa (watuk, kuping tuli, lara netra, padharan) ↔ `disease_ontology.json`. **Soal publikasi**: manuscript v08 (`MANUSCRIPT.md`, ~7K kata) sudah ditulis tapi **JANGAN target J. Ethnopharmacology** — JEP desk-reject paper murni komputasional tanpa wet-lab (verified 25 Mei); venue cocok = Frontiers in Pharmacology / J. Biomedical Informatics / Knowledge-Based Systems / Information Fusion. User pilih tanpa deadline → paper **menunggu sampai lebih kuat** (L1 bisa menambah layer historis). **4 cacat manuscript ditunda** (konklusi §6 pra-v08; figures 19/20 stale tanpa generator; figure 1 caption; formatting). Konteks lain kalau perlu: TRIAGE.md, NOTES_2026-04-20_*.md, NOTES_2026-05-0{3,7}_*.md, data_audit_jamu_sources.md (§5 sumber teks historis). State analitis lain (background, sudah selesai): KG v08 ontology-split (6,923 TREATS clinical + 1,387 HAS_USE + 407 ETHNOBOTANICAL_USE + 214 APPLIED_TO; gap 85.56% struktural); 11 mazhab + 5 bridge herbs; forbidden pairs Z=37.97; figures 00–17, 21–22; pipeline end-to-end reproducible. Arah analitis L2 yang masih menggantung (kapan-kapan, bukan prioritas): bridge herb investigation, synergy prediction, case study Piperaceae/S8, PubMed query-quality (hipotesis 88.5%). Tujuan HKI (Hak Cipta DJKI Program Komputer + Basis Data) masih relevan; technical bar terpenuhi. Prinsip user: **santai dalam waktu, serius dalam metodologi** — peneliti boleh salah/gagal/pivot asal tidak bohong. Jangan buru-buru submit paper. Tiga guardrails (CLAUDE.md + SCHEMA §G1–G3): (i) edit MANUSCRIPT.md tanpa deliberasi dulu, (ii) analisis baru tanpa robustness test, (iii) narrow exploration 3+ sesi tanpa zoom-out. Sesi 25 Mei membuka L1 (frontier yang lama dihindari) — jadi prinsip (iii) sudah terjawab; lanjutkan L1 mendalam dengan tenang.
